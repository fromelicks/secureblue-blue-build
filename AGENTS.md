...
| GDM greeter on the wrong display | [`docs/gdm-external-monitor.md`](docs/gdm-external-monitor.md) | The greeter is its own mutter with its own layout, and on GDM 50 it reads `/var/lib/gdm/seat0/config/monitors.xml` — **not** the universally documented `/var/lib/gdm/.config/`, which the dynamic-user migration left behind. `/var` cannot ship in the image, so the layout lives in `/usr/share/fromelicks/` and a oneshot copies it `Before=gdm.service`. |
