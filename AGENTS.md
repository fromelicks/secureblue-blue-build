...
| Lock-screen freeze on display blank | [`docs/nvidia-dpms-lock-hang.md`](docs/nvidia-dpms-lock-hang.md) | 2026-09-21: gsd-power's idle blank (`PowerSaveMode`) on an NVIDIA-driven HDMI display left gnome-shell's KMS thread in `D` at `nvkms_ioctl_from_kapi_try_pmlock`. This was *not* a suspend. SIGTERM cannot reach a thread in `D`, so recovery failed three times and a hard power-off followed. The hang monitor now triggers a sysrq-w blocked-task dump (via `fromelicks-sysrq-show-blocked.service`, because the monitor's `ProtectKernelTunables=yes` makes the trigger read-only) and, if the thread is still in the NVIDIA driver 60 s after SIGTERM, syncs and runs an orderly reboot. Lock holder still unidentified. |
