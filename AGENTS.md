...
| WebRTC voice over a split-tunnel VPN | [`docs/webrtc-vpn-ice-candidates.md`](docs/webrtc-vpn-ice-candidates.md) | Discord (Vesktop) hung at "DTLS Connecting" with a split-tunnel WireGuard up. libwebrtc classifies interfaces by name (prefix + digits only), ranks unrecognised ones *above* wifi alphabetically, and nominated an ICE candidate on the tunnel address — which black-holes, since its packets leak out wifi with a foreign source. Fix: interface name `tun0` (`wg0` is the worst possible name) plus `ujust wg-confine <profile>`, which confines the tunnel address to the tunnel's own routes with NetworkManager-owned policy rules in a dedicated table. Never put an unreachable route in a table a `suppress_prefixlength` rule reaches: it took all traffic down in testing. |
...
| **MATLAB R2026a + Simulink** | manifest + setup script in image, install at runtime | `/etc/distrobox/matlab.ini` template + `/etc/distrobox/matlab-products.txt` + `/usr/libexec/fromelicks-matlab-box-setup`; driven by `ujust matlab-box`. See [`docs/matlab-distrobox.md`](docs/matlab-distrobox.md) | Needs `set-container-userns on` (constraint 4). MATLAB has no Wayland backend, but constraint 7's toggle only passes `--no-x11` to gnome-shell — `/usr/bin/Xwayland` stays, so Niri (xwayland-satellite) and xpra both work with it left off; only the GNOME session needs `set-xwayland on`. Base is `quay.io/toolbx-images/ubuntu-toolbox:24.04`, already trusted by policy (constraint 1), so no policy overlay. ~50 GB, installed onto a host volume so `distrobox rm` does not cost a re-download. Network license via `MLM_LICENSE_FILE`; host netns means VPN routing just works — never set `unshare_netns`. |
