...
| Lock-screen freeze on display blank | [`docs/nvidia-dpms-lock-hang.md`](docs/nvidia-dpms-lock-hang.md) | 2026-09-21: gsd-power's idle blank (`PowerSaveMode`) on an NVIDIA-driven HDMI display left gnome-shell's KMS thread in `D` at `nvkms_ioctl_from_kapi_try_pmlock`. This was *not* a suspend. SIGTERM cannot reach a thread in `D`, so recovery failed three times and a hard power-off followed. The hang monitor now triggers a sysrq-w + sysrq-l dump before signalling (via `fromelicks-sysrq-show-blocked.service`, because the monitor's `ProtectKernelTunables=yes` makes the trigger read-only). If the thread is still in the NVIDIA driver after SIGTERM, it sends SIGKILL at 15 s and runs an orderly reboot at 60 s, with no explicit `sync`, which can hang unkillably. "In the NVIDIA driver" is decided by kallsyms **module ownership**, not a name prefix. **Root cause found 2026-09-23** (third occurrence, first with the recovery deployed): the holder is the `nvidia-modeset/` kthread running a **DIFR** (Display Idle Frame Refresh) prefetch — `nvDIFRPrefetchSurfaces` → `nvPushKickoff` → stuck in `nvWriteGpEntry` — while holding the global nvkms lock, so every compositor modeset queues behind it. It was *running*, so `sysrq-l` found it and `sysrq-w` alone would not have. SIGKILL does not free the victim. No DIFR knob exists outside GSP firmware. That reboot then stalled ~20 min in per-unit kill ladders, partly on the monitor's own unkillable debugfs `state` reads; fixed by skipping that read during a wedge, `TimeoutStopSec=10s`, and a `reboot.target` drop-in cutting `JobTimeoutSec` from 30 min to 180 s. |
...
1. **Container signature policy is default-reject.** `/etc/containers/policy.json` rejects any
   ...
   `quay.io/fedora-ostree-desktops`, `registry.access.redhat.com`. Note `quay.io/toolbx-images`
   is trusted but **no longer pullable** — the namespace went auth-only, which is why the MATLAB
   box moved to `registry.access.redhat.com/ubi9/toolbox` (see `docs/matlab-distrobox.md`).
   To pull dev/other images, add policy entries. **DECISION
...
8. **hardened_malloc is globally preloaded.** Some apps (Steam) require it disabled per-app.
   ...
   **Claude Desktop** aborts the same way; `/usr/bin/claude-desktop` is a
   `with-standard-malloc` wrapper. Its env probe runs `env -0`, not Electron, so it needs
   no `profile.d` hook. See [`docs/claude-desktop.md`](docs/claude-desktop.md).
...
15. **GNOME reads caches, not directories, and on ostree they never look stale.**
    `/usr/share/icons/hicolor/icon-theme.cache` and
    `/usr/share/applications/mimeinfo.cache` are generated by RPM scriptlets during the
    `dnf` module. Anything a *later* `script` or `files` module drops into those
    directories is therefore invisible to the desktop: a blank app-grid icon, an
    unregistered `x-scheme-handler/`, a missing MIME association. GTK's usual safety
    net — cache older than its directory means rescan — cannot fire, because ostree
    sets every mtime to epoch 0, so the cache always looks current. Any module that
    adds a `.desktop` file or an icon after the dnf step must end with
    `gtk-update-icon-cache --force` and/or `update-desktop-database`, and should assert
    the new entry is really in the cache: the failure is silent, and the app still
...
| **Claude Desktop** | image | `install-claude-desktop.sh` unpacks Anthropic's `.deb` (no RPM exists) after verifying pinned key → `InRelease` → `Packages` → `.deb` SHA256; newest version each build | hardened_malloc opt-out (constraint 8). `chrome-sandbox` SUID bit dropped: the userns sandbox works. Cowork untested (needs `/dev/vhost-vsock` + `kvm` group). See [`docs/claude-desktop.md`](docs/claude-desktop.md). |
| **MATLAB R2026a + Simulink** | manual install at runtime, run from a distrobox | `/etc/distrobox/matlab.ini` template + `fromelicks-matlab-box-setup` (container init hook) + `fromelicks-xwayland-isolated`; MATLAB installed by hand into `~/.local/share/matlab-distrobox/R2026a`, then `ujust matlab-box` / `ujust matlab`. See [`docs/matlab-distrobox.md`](docs/matlab-distrobox.md) | Needs `set-container-userns on`; does **NOT** need `set-xwayland on` — a private Xwayland via `xwayland-satellite` works under Mutter with `--no-x11` still on, with hardware GL. `hardened_malloc` **kills** MATLAB's CEF UI process, installer included — every entry point must `unset LD_PRELOAD`. distrobox's bare `SHELL=bash` makes every MATLAB `system()`/`mex`/Coder build fail with 127 — the manifest sets `SHELL=/bin/bash` container-wide. `mpm --source` cannot install from the release ISO (bogus "Download failed"; really a signature error) — use the ISO's own installer. Base is `registry.access.redhat.com/ubi9/toolbox` (trusted + MathWorks-supported); `quay.io/toolbx-images` went auth-only. ~25 GB on a host volume. Network license via `MLM_LICENSE_FILE`; host netns means VPN routing just works — never set `unshare_netns`. |
